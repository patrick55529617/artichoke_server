-- FUNCTION: public.counter_complement_hotfix_v2_1(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, integer, integer, integer)

-- DROP FUNCTION public.counter_complement_hotfix_v2_1(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, integer, integer, integer);

CREATE OR REPLACE FUNCTION public.counter_complement_hotfix_v2_1(
	_antenna_1 text,
	_rssi_1 integer,
	_antenna_2 text,
	_rssi_2 integer,
...
format('
		WITH RAW AS(
		    ...
		    WHERE NOT EXISTS (SELECT 1
		                      FROM random_samsung_candidate(''%1$s'',%14$s,''%5$s'',
		                            ''%6$s'',%7$L,%12$s,%13$s) AS B
		...
		_window_size, _offset, _rolling_minute, _theta,
    _random_rssi);
END;
