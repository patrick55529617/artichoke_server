-- FUNCTION: public.counter_union_seamless(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, double precision, timestamp with time zone, integer, integer, double precision)

-- DROP FUNCTION public.counter_union_seamless(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, double precision, timestamp with time zone, integer, integer, double precision);

CREATE OR REPLACE FUNCTION public.counter_union_seamless(
	_antenna_1 text,
	_rssi_1 integer,
	_antenna_2 text,
	_rssi_2 integer,
	...
	_random_rssi_1 integer,
	_random_rssi_2 integer,
...
WHEN _timefrom::date >= seamless_end_time OR date_part('day', seamless_interval) = 0 THEN
  ...
  RETURN QUERY SELECT * FROM public.counter_union(_antenna_1, _seamless_new_rssi_1, _antenna_2, _seamless_new_rssi_2,
...
WHEN _timeto::date < seamless_start_time THEN
  -- not starting seamless
	RETURN QUERY SELECT * FROM public.counter_union(_antenna_1, _rssi_1, _antenna_2, _rssi_2,
...
ELSE
  RETURN QUERY SELECT t.time, (weight*count_new + (1-weight)*count_old)::bigint AS "customers"
      	       ...
      	       FROM (
        	       ...
        	       FROM public.counter_union(_antenna_1, _rssi_1, _antenna_2, _rssi_2,
                                             _timefrom, _timeto, _android_oui, _android_ratio, _wifi_ratio,
                                             _random_rssi_1, _random_rssi_2) AS OLD,
                   public.counter_union(_antenna_1, _seamless_new_rssi_1, _antenna_2, _seamless_new_rssi_2,
                                       _timefrom, _timeto, _android_oui, _seamless_new_ratio, _wifi_ratio,
                                       _random_rssi_1, _random_rssi_2) AS NEW
...
ALTER FUNCTION public.counter_union_seamless(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, double precision, timestamp with time zone, integer, integer, double precision)
    OWNER TO artichoke;
