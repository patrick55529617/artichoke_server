-- FUNCTION: public.counter_hotfix_v2(text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer)

-- DROP FUNCTION public.counter_hotfix_v2(text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer);

CREATE OR REPLACE FUNCTION public.counter_hotfix_v2(
	_antenna text,
	_rssi integer,
...
RETURN QUERY EXECUTE
    ...
    WITH RAW AS(
        ...
        WHERE NOT EXISTS (SELECT 1
                          FROM random_samsung_candidate(''%1$s'',%4$s,''%2$s'',
