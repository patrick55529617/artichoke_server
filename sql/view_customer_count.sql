﻿-- View: public.customer_count

-- DROP VIEW public.customer_count;

CREATE OR REPLACE VIEW public.customer_count AS
 SELECT tab.site_id,
    tab."time" AS ts_hour,
    tab.count
   FROM ( SELECT people_count.site_id,
            people_count."time",
            people_count.count
           FROM people_count
        UNION
       ...
       -- HOLA Petite
       UNION
         SELECT '1B55'::text AS site_id,
            zhonghe_petite."time" AS ts_hour,
            zhonghe_petite.customers AS count
           FROM zhonghe_petite(date_trunc('day'::text, timezone('Asia/Taipei'::text, now()))::timestamp with time zone) zhonghe_petite("time", customers)
       UNION
         SELECT '1B57'::text AS site_id,
            jhongli_petite."time" AS ts_hour,
            jhongli_petite.customers AS count
           FROM jhongli_petite(date_trunc('day'::text, timezone('Asia/Taipei'::text, now()))::timestamp with time zone) jhongli_petite("time", customers)
        UNION
         SELECT '1B59'::text AS site_id,
            neili_petite."time" AS ts_hour,
            neili_petite.customers AS count
           FROM neili_petite(date_trunc('day'::text, timezone('Asia/Taipei'::text, now()))::timestamp with time zone) neili_petite("time", customers)
        UNION
         SELECT '1B61'::text AS site_id,
            nankan_petite."time" AS ts_hour,
            nankan_petite.customers AS count
           FROM nankan_petite(date_trunc('day'::text, timezone('Asia/Taipei'::text, now()))::timestamp with time zone) nankan_petite("time", customers)
        ) tab
...
ALTER TABLE public.customer_count
    OWNER TO artichoke;

GRANT SELECT ON TABLE public.customer_count TO dw;
GRANT SELECT ON TABLE public.customer_count TO shiny;
GRANT ALL ON TABLE public.customer_count TO artichoke;
GRANT SELECT ON TABLE public.customer_count TO echoi;
