-- FUNCTION: public.counter_complement_hotfix_v2(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, integer, integer)

-- DROP FUNCTION public.counter_complement_hotfix_v2(text, integer, text, integer, timestamp with time zone, timestamp with time zone, character[], double precision, double precision, integer, integer, integer, integer);

CREATE OR REPLACE FUNCTION public.counter_complement_hotfix_v2(
	_antenna_1 text,
	_rssi_1 integer,
	_antenna_2 text,
	_rssi_2 integer,
    ...
    format('
		WITH RAW AS(
		    ...
		    WHERE NOT EXISTS (SELECT 1
		                      FROM random_samsung_candidate(''%1$s'',%2$s,''%5$s'',
